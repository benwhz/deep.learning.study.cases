...
# Path to the images
IMAGES_PATH = "./data/Flicker8k/Images/"
...
def load_captions_data(filename):
    ...
    with open(filename) as caption_file:
        ...
        for line in caption_data:
            ...
            if os.path.exists(img_name) and img_name.endswith("jpg") and img_name not in images_to_skip:
...
# Load the dataset
captions_mapping, text_data = load_captions_data("./data/Flicker8k/Flickr8k.token.txt")

# Split the dataset into training and validation sets
train_data, valid_data = train_val_split(captions_mapping)
print("Number of training samples: ", len(train_data))
print("Number of validation samples: ", len(valid_data))
print('text data length = ', len(text_data))
...
for key in train_data.keys():
    img, vect = process_input(key, train_data[key])
    break
...
def get_cnn_model():
    ...
    print(base_model.input.shape, base_model.output.shape)
...
model.add(keras.Input(shape=(100,1280,)))
model.add(encoder)
model.summary()
'''
...
class TransformerDecoderBlock(layers.Layer):
    ...
    def call(self, inputs, encoder_outputs, training, mask=None):
        ...
        print('inputs shape', inputs.shape, encoder_outputs.shape)
...
input = keras.Input(shape=(SEQ_LENGTH,))
encoder_outputs = keras.Input(shape=(100,512))
y = decoder(input, encoder_outputs, training=False)
model = keras.Model(input, y)
model.summary()
#exit()
'''
...
class ImageCaptioningModel(keras.Model):
    ...
    def _compute_caption_loss_and_acc(self, img_embed, batch_seq, training=True):
        ...
        print(batch_seq_inp.shape, encoder_out.shape)
